...
const DeveloperPanel: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'companies' | 'support'>('companies');
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-black text-slate-800 flex items-center gap-3 uppercase tracking-tighter">
          <i className="fa-solid fa-chart-line text-blue-600"></i> Painel de Controle
        </h2>

        <div className="flex bg-slate-100 p-1 rounded-xl">
          <button
            onClick={() => setActiveTab('companies')}
            className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'companies' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
          >
            Empresas
          </button>
          <button
