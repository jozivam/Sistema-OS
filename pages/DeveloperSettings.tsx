...
const DeveloperSettings: React.FC = () => {
  ...
  const devUsers = useMemo(() => {
    return data.users.filter(u => u.role === UserRole.DEVELOPER);
  }, [data.users]);
  ...
  const handleSaveUser = (e: React.FormEvent) => {
    ...
    if (editingUser) {
      ...
      // Sincronizar com o usuário logado se ele estiver editando o próprio perfil
      if (currentData.currentUser && editingUser.id === currentData.currentUser.id) {
        isEditingSelf = true;
      }
    ...
    // Se estiver editando a si mesmo, limpamos o currentUser para forçar novo login
    const finalData = { 
      ...currentData, 
      users: updatedUsers, 
      currentUser: isEditingSelf ? null : currentData.currentUser 
    };
    ...
    // Se o usuário alterou o próprio perfil, redirecionamos para login limpando o estado global
    if (isEditingSelf) {
